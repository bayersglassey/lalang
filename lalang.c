...
bool object_getter(object_t *self, const char *attr, vm_t *vm) {
    type_t *type = self->type;
    bool ok = type->getter? type->getter(self, attr, vm): false;
    if (ok) return true;
    else {
...
bool int_getter(object_t *self, const char *attr, vm_t *vm) {
    bool is_binop = true;
    char op;
    if (!strcmp(attr, "+")) op = '+';
...
bool list_getter(object_t *self, const char *attr, vm_t *vm) {
    list_t *list = self->data.ptr;
    if (!strcmp(attr, "new")) {
    ...
    } else if (!strcmp(attr, "pop")) {
        vm_push(vm, list_pop(list));
    } else if (!strcmp(attr, "push")) {
        object_t *value = vm_pop(vm);
...
bool dict_getter(object_t *self, const char *attr, vm_t *vm) {
    dict_t *dict = self->data.ptr;
    if (!strcmp(attr, "new")) {
...
bool func_getter(object_t *self, const char *attr, vm_t *vm) {
    func_t *func = self->data.ptr;
    if (!strcmp(attr, "@")) {
...
int main(int n_args, char **args) {
    ...
    while (true) {
        ...
        code_t *code = vm_compile(vm, line);
        vm_eval(vm, code);
    }
    return 0;
}
